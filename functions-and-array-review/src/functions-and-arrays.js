...
function findLongestWord(arrayOfWords) {
  // let most = Math.max(arrayOfWords.map((eachWord)=> eachWord.length));
  // let result = arrayOfWords.filter((eachWord)=>{
  //   return eachWord.length === most;
  // }) 
  // return result[0];
...
function averageNumbers(arrayOfNums) {
  if(!arrayOfNums.length) return null;
...
// Bonus - Iteration #4.1
function avg(arrayOfStuff) {
  if(!arrayOfStuff.length) return null;
  return sum(arrayOfStuff) / arrayOfStuff.length;
}
